...
public class ADIS16448_IMU extends GyroBase implements Gyro, PIDSource, LiveWindowSendable {
  ...
  // Offsets
  private double relative_north_yaw = 0.0;
  private double relative_north_roll = 0.0;
  private double relative_north_pitch = 0.0;
  private AtomicBoolean m_freed = new AtomicBoolean(false);

  private SPI m_spi;
  private DigitalOutput m_reset;
  private DigitalInput m_interrupt;
  private boolean upsidedown_mount = true;
   ...
   * Constructor.
   */
  public ADIS16448_IMU(Axis yaw_axis, AHRSAlgorithm algorithm, boolean upsidedown_mount) {
    m_yaw_axis = yaw_axis;
    m_algorithm = algorithm;
    this.upsidedown_mount = upsidedown_mount;
  ...
  public ADIS16448_IMU(Axis yaw_axis) {
    this(yaw_axis, AHRSAlgorithm.kComplementary, true);
  }
  ...
  public ADIS16448_IMU() {
    this(Axis.kZ, AHRSAlgorithm.kComplementary, true);
  }
  ...
  public void reset() {
    synchronized (this) {
      ...
      m_integ_gyro_y = 0.0;
      m_integ_gyro_z = 0.0;
      relative_north_yaw = this.getYaw();
      relative_north_roll = this.getRoll();
      relative_north_pitch = this.getPitch();
    }
  }
  ...
  public double applyRelativeCorrection(double true_heading, double original_baseline){
	  
	  double corrected_heading = true_heading - original_baseline;
	  if (corrected_heading < -180)	  {
		  corrected_heading+= 360;
	  }
	  else if (corrected_heading > 180){ 
		  corrected_heading-=360;
	  }
  ...
  public double getRelativePitch() {
	    return applyRelativeCorrection(m_pitch,relative_north_pitch);
  }

  public double getRelativeRoll() {
     return applyRelativeCorrection(m_roll,relative_north_roll);
   }

   public double getRelativeYaw() {
	   return applyRelativeCorrection(m_yaw,relative_north_yaw);
}
